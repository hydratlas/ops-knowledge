...
- **VictoriaMetrics**: `restic restore` でパートを取り出し、VM を停止してデータディレクトリーへ戻して
  再起動する。`vmrestore` は vmbackup 形式のリモートストレージを前提とするため本方式の復旧手段では
  ない。本ジョブが restic に投入するのは snapshot 直下（データ本体への symlink ツリー）ではなく、
  その実体（`data/{big,small,indexdb}/snapshots/<snap>`、ハードリンク）を安定パスへ bind したもので
  ある（`templates/restic-hooks/victoriametrics-pre.sh.j2` 冒頭の実機検証メモを参照）。手順は実機で
  確定済み（2026-06-28 非破壊リハーサル。記録 `docs/engineering-records/202606/20260628-2210-victoriametrics-restore-rehearsal.md`）。

  restic スナップショット内のツリーは `/mnt/vm-snapshot/{big,small,indexdb}/<partition>/<part>` と
  `/mnt/vm-snapshot/metadata/minTimestampForCompositeIndex` であり、これを live のデータディレクトリー
  （`/home/monitoring/.local/share/victoria-metrics-data`）の `data/{big,small,indexdb}/<partition>/<part>`
  と `metadata/minTimestampForCompositeIndex` へ対応付けて戻す。単一 VM snapshot 由来のため3系統は
  相互に整合する。`cache`・`snapshots`・`tmp`・`flock.lock` は VM が起動時に再生成するため復元不要。
  restore したファイルは元の所有者（`monitoring:monitoring`、uid 999）を保持し、本番コンテナの
  `UserNS=keep-id`（uid 999）とそのまま整合する。pre-hook の恒久修正（`rm -rf "$BIND_BASE/metadata"` の
  追加）により 2026-06-28 以降のバックアップでは `metadata/` に入れ子が生じない。修正前に取得した
  スナップショットに入れ子が残る場合があるが、復旧では top-level の `metadata/minTimestampForCompositeIndex`
  を使えばよく、入れ子は無視してよい（実害なし。retention で自然に世代一巡で消える）。

  確定手順（monitoring 上で実施。restic は root、VM 操作は monitoring ユーザー）:

  1. **restore**: `RESTIC_REPOSITORY=/mnt/backup-dst/victoriametrics RESTIC_PASSWORD_FILE=/etc/restic/repo.pass restic restore latest --target /var/tmp/vm-restore`（出力は `/var/tmp/vm-restore/mnt/vm-snapshot/...`）。
  2. **VM 停止**: `systemctl --user stop victoria-metrics`（quadlet 生成ユニット。`Restart=always` のため必ず停止してから入れ替える）。
  3. **データ入れ替え**: 既存 `data/`・`metadata/` を退避（または削除）し、restore した `big`/`small`/`indexdb` を `data/` 配下へ、`metadata/minTimestampForCompositeIndex` を `metadata/` へ配置する。
  4. **起動・確認**: `systemctl --user start victoria-metrics` 後、`localhost:8428/api/v1/status/tsdb` の totalSeries と過去時点の `count(up)` 等が復元前と一致することを確認する。

  authKey に関する注意: 本復旧は `restic restore` のみで完結し VM の snapshot API を叩かないため
  `-snapshotAuthKey` は不要。ただし復旧の前後で手動に snapshot を作成・全削除する（`/snapshot/create`・
  `/snapshot/delete_all`）場合は認証必須化済みのため、キーファイル
  `/home/monitoring/.config/victoria-metrics/snapshot-auth.key` の値を `authKey` クエリーで付与する
  （restic の pre/post フックは同ファイルから自動付与する）。
