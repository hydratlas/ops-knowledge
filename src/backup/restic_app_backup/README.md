...
- VM/VL の安定パス＋ハードリンク・スナップショット方式（親照合で新規 part のみ再ハッシュ）
...
### 安定パス＋ハードリンク・スナップショット方式（VM/VL）

VictoriaMetrics / VictoriaLogs の data part は確定後に不変であり、各々のスナップショット API はその part を**ハードリンク**でスナップショット・ディレクトリーに並べる（データ複製なし・瞬時）。これを毎回同じ安定パス（`/mnt/vm-snapshot` 等）へ `mount --bind` で提示して restic に食わせると、未変更 part は restic の親照合 `(path, mtime, size, inode)` で一致し、restic は新規 part だけを読んでハッシュする。`vmbackup -origin` のような自前増分は不要（restic が世代間差分を担う）。MariaDB・FreeIPA は出力が毎回新規ファイル・小容量のため安定パス最適化は使わず、使い捨て `$STAGING` のままとする。
...
| `pre_command`  | 整合点を作るシェル断片（`$STAGING` 利用可）                      | はい   |
| `post_command` | 後始末のシェル断片（VM/VL の snapshot 削除・umount 等）          | いいえ |
| `source_path`  | `restic backup` に渡すパス（省略時は `$STAGING`）                | いいえ |
...
- **VictoriaMetrics**: `restic restore` で `big`/`small`/`indexdb`/`metadata` を取り出し、VM を停止して
  ...
  （`group_vars/monitoring.yml` の job 定義の実機検証メモを参照）。`vmrestore` は vmbackup 形式の
  リモートストレージを前提とするため本方式の復旧手段ではない。具体的な復旧手順は実機で確定する。
- **VictoriaLogs**: 本ジョブは未対応（partition snapshot API がパーティション別の多数の
  snapshot を返すため再設計が必要。`group_vars/monitoring.yml` の保留メモを参照）。
