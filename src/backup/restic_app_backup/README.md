...
- **VictoriaMetrics**: `restic restore` でパートを取り出し、VM を停止してデータディレクトリーへ戻して
  ...
  `UserNS=keep-id`（uid 999）とそのまま整合する。なお過去の pre-hook 仕様により restore したツリーの
  `metadata/` に入れ子（`metadata/metadata/minTimestampForCompositeIndex`）が混じる場合があるが、復旧では
  top-level の `metadata/minTimestampForCompositeIndex` を使えばよい（入れ子は無視してよい軽微なクルフト）。
