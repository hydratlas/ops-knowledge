...
| 変数名 | 説明 | デフォルト値 | 必須 |
|--------|------|-------------|------|
| `ceph_version` | 使用するCephのバージョン | `squid` | いいえ |
| `enable_ceph_repo` | Ceph no-subscriptionリポジトリを作成するか | `false` | いいえ |

**利用可能なCephバージョン:**
- `reef` - Ceph 18.x（サポート終了間際）
- `squid` - Ceph 19.x（デフォルト）
