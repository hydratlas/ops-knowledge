...
- パッシブ（VIP なし）のときのみ、read-only でマウントした VIP の NFS からローカルの集約先へ rsync でプルする。`-H` でハードリンクを保存し（vmbackup の `-origin` がアクティブの ext4 上で世代間の同一パートをハードリンク共有して省スペース化するため、`-H` が無いとパッシブ側で世代ごとにフルコピーへ膨張する）、`--update` を付けタイムスタンプの巻き戻しを防ぎ、`--delete` は付けず世代の巻き添え削除を防ぐ。書き込み途中の `.tmp-*` は除外する。
