...
| 変数名                    | 既定値 | 説明                                                                                                       |
| ------------------------- | ------ | ---------------------------------------------------------------------------------------------------------- |
| `nfs_server_rdma_enabled` | `false` | NFSoRDMA を有効化する。`/etc/nfs.conf` の `[nfsd] rdma=y` と `rdma-port` を設定し、`svcrdma` モジュールを常時ロードする。 |
| `nfs_server_rdma_port`    | `20049` | NFSoRDMA のリッスンポート。                                                                                |
| `nfs_server_threads`      | `null` | `nfsd` カーネルスレッド数。`null` のときディストリビューションの既定（Ubuntu 24.04 は 8）に従う。高帯域 NIC では 64〜128 を指定する。 |
