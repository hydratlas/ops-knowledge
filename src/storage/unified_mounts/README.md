...
- **自動アンマウント**: 一定時間アクセスがないと自動的にアンマウントされる（`TimeoutIdleSec`、デフォルト60秒。`unified_mounts_nfs_idle_timeout`で変更可能）
...
| `opts` | マウントオプション（自動付与のデフォルトはなく、そのまま展開される。NFSでは`hard,nofail`を推奨） | なし | NFSでは実質必須 |
...
systemd timerを使用して定期的にNFSマウントの状態を監視する。マウントポイントへの`stat`がstale file handle（ESTALE）等のエラーを返した場合に限り、遅延アンマウント（umount -l）を実行し、automountユニット（LXCではmountユニット）を再起動する。

なお、`stat`が**タイムアウト**した場合（サーバーのダウンや一時的なネットワーク断）は強制アンマウントを行わない。`hard`マウントはサーバー復帰時に開いているファイルごと自然回復するため、ここで遅延アンマウントするとかえって回復可能なファイルハンドルを失うからである。この機能が対象とするのは、あくまでrsyncベースのフェイルオーバー等でファイルハンドルが無効化されESTALEとなるケースである。
...
2. serviceがスクリプトを実行
3. スクリプトが各NFSマウントポイントに対して`stat`コマンドでアクセスを試みる（`timeout`で時間制限）
4. 結果を終了コードで判別する
   - 成功: 正常。何もしない
   - タイムアウト（終了コード124）: サーバー無応答。`hard`マウントの自然回復に委ねるため、強制アンマウントしない
   - その他のエラー（ESTALE等）: 遅延アンマウント（umount -l）を実行
5. アンマウント後、VM（automount）では次回アクセス時に新しい接続が確立され、LXC（mountユニット）では即座に再マウントされる
