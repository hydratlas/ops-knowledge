...
この設定により、`<subdomain>.<domain>` への SSH 接続が Tunnel を経由してサーバーの SSH サーバー（ポート 22）に転送される。クライアント側の SSH 設定については [Cloudflare Tunnel SSH 設定ガイド](../../../docs/design/cloudflare-tunnel-ssh.md) を参照。
...
- [Cloudflare Tunnel SSH 設定ガイド](../../../docs/design/cloudflare-tunnel-ssh.md) - クライアント側の SSH 設定
