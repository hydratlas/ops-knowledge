...
`site.yml`では`tofu_virtual:general`に対して本ロールを適用しており、実際に有効化するかは`kernel_modules_disable_enabled`(`group_vars`側)で決める。LXC(`tofu_lxc`)はホストカーネルを共有するため対象外、VyOS(`rt-*`)は独自イメージのため対象外としている。
