...
- APIトークンの作成手順:
  ...
  2. Datacenter > Permissions > API Tokens でトークン作成（User: `prometheus@pve`、Token ID: `exporter`）
  3. Datacenter > Permissions の Add > API Token Permission でロールを割り当て（Path: `/`、API Token: `prometheus@pve!exporter`、Role: `PVEAuditor`）
- Ansible変数
...
| `pve_exporter_pve_user`            | `prometheus@pve`                                   | Proxmox VE APIユーザー（必須）   |
| `pve_exporter_pve_token_name`      | `exporter`                                         | APIトークン名（必須）            |
| `pve_exporter_pve_token_value`     | ``                                                 | APIトークン値（必須）            |
  ...
  roles:
    - role: services.monitoring.pve_exporter
      vars:
        ...
        pve_exporter_pve_token_name: "exporter"
        pve_exporter_pve_token_value: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
```

大規模環境でconfigコレクターを無効化する例:
  ...
  roles:
    - role: services.monitoring.pve_exporter
      vars:
        pve_exporter_pve_user: "prometheus@pve"
        pve_exporter_pve_token_name: "exporter"
        pve_exporter_pve_token_value: "{{ vault_pve_exporter_token }}"
        pve_exporter_collector_config: false
```
