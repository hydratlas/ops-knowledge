...
FreeIPA クライアントに登録済みのホストで SSSD の共通設定を整えるロール

## 概要

### このドキュメントの目的
このロールは、`ipa-client-install` が生成した `/etc/sssd/sssd.conf` を、運用上望ましい構成に揃えます。具体的には、`[sssd]` セクションの `services=` 行を削除して SSSD を socket activation 方式に移行し、ドメインセクションに `sudo_provider = ipa` を追加して FreeIPA で集中管理された sudo ルールを利用可能にします。Ansibleによる自動設定と手動設定の両方の方法について説明します。

### 実現される機能
- `services=` 行の除去による socket activation 化（`sssd-*.socket` の `failed` 解消）
...
# services= 行は記述しない（socket activation に任せる）
domains = example.com
