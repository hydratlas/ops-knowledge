...
| 変数名                             | デフォルト値    | 説明                                             |
| ---------------------------------- | --------------- | ------------------------------------------------ |
| `node_exporter_version`            | `1.11.1`        | Node Exporterのバージョン                        |
...
| `node_exporter_firewalld_sources`  | `[]`            | firewalld 稼働ホストで待受ポートを開放する送信元リスト |
...
### firewalld 稼働ホストでのポート開放

FreeIPA サーバー（idm）のように firewalld を稼働させているホストでは、`node_exporter_firewalld_sources` に scrape 元のセグメント（CIDR またはアドレス）を列挙することで `9100/tcp` を開放できる。空（既定）のホストや firewalld が非稼働のホストでは何もしない。

```yaml
node_exporter_firewalld_sources:
  - "10.120.0.0/16"
```
