...
| `victorialogs_partition_auth_key`  | `""`                                             | partition 管理 API（`-partitionManageAuthKey`）の認証キー。空文字なら無効（フラグ・マウントとも付与しない）。環境側で `vault_vl_partition_auth_key` を引き渡す |
...
Exec='-retentionPeriod=7d' '-partitionManageAuthKey=file:///etc/partition-auth.key'
```
