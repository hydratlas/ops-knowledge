...
| `victorialogs_partition_auth_key`  | `""`                                             | 管理系 API の共通認証キー。partition（`-partitionManageAuthKey`）に加え force_flush（`-forceFlushAuthKey`）・force_merge（`-forceMergeAuthKey`）・log_new_streams（`-logNewStreamsAuthKey`）・flags（`-flagsAuthKey`）・pprof（`-pprofAuthKey`）へ同一キーを付与する。空文字なら無効（フラグ・マウントとも付与しない）。環境側で `vault_vl_partition_auth_key` を引き渡す |
...
VictoriaLogs は partition 管理 API（`/internal/partition/snapshot/create`・`/internal/partition/snapshot/delete`・`/internal/partition/snapshot/list`・`/internal/partition/snapshot/delete_stale`・`/internal/partition/list`）とデータ系 API（`/insert/...`・クエリー）を同一ポート（9428）で提供する。無認証のままだと int セグメントの任意ホストから snapshot の作成・削除が可能になるため、`-partitionManageAuthKey` で `/internal/partition/*` 配下のみを認証必須化する。認証キーはログ取り込み・クエリーには影響しない。

partition 以外にも無認証では破壊的・デバッグ用途の管理系エンドポイントが残るため、同一キーで一括して認証必須化する。`/internal/force_flush`（`-forceFlushAuthKey`。インメモリーデータの即時フラッシュ）・`/internal/force_merge`（`-forceMergeAuthKey`。手動コンパクション・マージ）・`/internal/log_new_streams`（`-logNewStreamsAuthKey`。新規ログストリーム記録のデバッグトグル）・`/flags`（`-flagsAuthKey`）・`/debug/pprof/*`（`-pprofAuthKey`）の 5 つを `-partitionManageAuthKey` と同じキーファイル（`/etc/partition-auth.key`）へ向ける。これらは人手（DR・障害解析）でのみ叩かれ自動化からは呼ばれないため、鍵を細分する権限分離の実益はなく、サービス内 1 鍵に統一する。キーファイル名は partition 専用に見えるが、実体は VictoriaLogs の管理系 API 共通鍵である。`-metricsAuthKey` は設定しない。`/metrics` は vmagent のスクレイプ対象であり、認証必須化するとスクレイプが壊れるためである。

キーは Vault（`vault_vl_partition_auth_key`）で暗号化管理し、コンテナへは `file://` でファイル経由で渡す（常駐プロセスの `ps` 引数露出を避けるため）。ホスト側キーファイルは `~/.config/victoria-logs/partition-auth.key`（monitoring 所有・`0600`）に配置し、`:ro,Z`（read-only かつリラベル）でコンテナへマウントする。`UserNS=keep-id` によりコンテナ内ではホストの monitoring uid にマップされ、`0600` でも読める。VM 側の snapshot 認証キーとは別ファイル・別 Vault 変数で管理する（最小権限・独立ローテーション）。
...
sudo -u monitoring mkdir -p /home/monitoring/.config/victoria-logs
printf '%s' "$KEY" | sudo -u monitoring tee /home/monitoring/.config/victoria-logs/partition-auth.key > /dev/null
sudo chmod 600 /home/monitoring/.config/victoria-logs/partition-auth.key
```

コンテナ定義（`victoria-logs.container`）の `[Container]` へ以下を追加する。

```ini
Volume=/home/monitoring/.config/victoria-logs/partition-auth.key:/etc/partition-auth.key:ro,Z
Exec='-retentionPeriod=7d' '-partitionManageAuthKey=file:///etc/partition-auth.key' '-forceFlushAuthKey=file:///etc/partition-auth.key' '-forceMergeAuthKey=file:///etc/partition-auth.key' '-logNewStreamsAuthKey=file:///etc/partition-auth.key' '-flagsAuthKey=file:///etc/partition-auth.key' '-pprofAuthKey=file:///etc/partition-auth.key'
```

### 動作確認・復旧時の操作

partition 管理 API を叩く際は `authKey` クエリーを付与する。既存クエリーを持たない `list`・`create` は `?authKey=`、既に `path=` を持つ `delete` は **`&authKey=`** で連結する（`?` を機械的に付与すると `delete?path=...?authKey=...` となり破綻する）。

```bash
AUTHKEY=$(cat /home/monitoring/.config/victoria-logs/partition-auth.key)
# 無キーでは非 200（401/403）、正キーで 200 となることを確認
curl -s -o /dev/null -w '%{http_code}' http://localhost:9428/internal/partition/snapshot/list
curl -s "http://localhost:9428/internal/partition/snapshot/list?authKey=$AUTHKEY"
# delete は path 既存クエリーへ &authKey で連結
curl -s "http://localhost:9428/internal/partition/snapshot/delete?path=<パス>&authKey=$AUTHKEY"
```
...
ローテーションは Vault 値を更新して再適用（コンテナ再起動）するだけでよい。無認証へ戻す場合は `victorialogs_partition_auth_key` を空（`""`）にして再適用すれば、フラグ・マウントとも外れキーファイルも配置されなくなる。
